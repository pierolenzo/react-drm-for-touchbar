...
import { ESC_KEY, FONTS } from '../config';
  ...
  return (
    <Box style={{ flex: 1, alignItems: 'stretch', gap: 6, paddingHorizontal: 8 , backgroundColor: '#000' }}>
      ...
      {showEsc && (
        ...
        >
          <Text  fontSize={FONTS.sizes.fnKeys} fontFamily={FONTS.families.monospace} style={{ fontWeight: '700' }}>esc</Text>
        </Button>
      )}
      ...
      {KEYS.map((key, i) => (
        ...
        >
          <Text  fontSize={FONTS.sizes.fnKeys} fontFamily={FONTS.families.monospace} style={{ fontWeight: '700' }}>{key}</Text>
        </Button>
      ))}
